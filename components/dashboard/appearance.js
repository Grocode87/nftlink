...
const Appearance = ({userData, setUserData}) => {
    ...
    const updateUserData = () => {
        if((name != userData.displayName) || (bio != userData.description)) { 
            const newUserData = {...userData, displayName: name || "", description: bio || ""}
        ...
        <div className='p-12 px-32'>
            <div>
                ...
                <div className='bg-white rounded-xl border shadow-lg p-4'>
                    ...
                    <input className='w-full border rounded-md p-2' defaultValue={userData.displayName} placeholder='Enter a display name' onBlur={updateUserData} onChange={e => {updateProfile(e.target.value, 'name')}}></input>

                    <p className='pt-8 pb-2'>Bio</p>
                    <textarea className='w-full h-40 border p-2 rounded-md' placeholder='Enter a bio to appear in your NFTinBio page' defaultValue={userData.description} onBlur={updateUserData} onChange={e => {updateProfile(e.target.value, 'bio')}}></textarea>
                </div>
            </div>
                            ...
                            {themes.map(({id, name, colors}) => {
                                return (
                                    <button onClick={() => changeTheme(id)}>
                                        <div className={"p-2 border-2 border-white rounded-lg " + (currTheme == id ? 'border-2 border-gray-300' : '')}>
                                            <div className={'mx-2 rounded-lg overflow-hidden text-center my-2 '}>
                                                {colors.length > 1 ? (
                                                    <div className='h-20' style={{background: `linear-gradient(45deg, ${colors[0]}, ${colors[1]}`}}></div>
                                                ) : (
                                                    <div className='h-20' style={{backgroundColor: colors[0]}}></div>
                                                )}
